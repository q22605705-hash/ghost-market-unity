...
## REQ-006 — 戰場背景圖（純美術，不改玩法）　　狀態：done（Claude 2026-07-05，已依模式切換）
...
## REQ-009 — 最後一批：ghoul / mage / brute 專屬 sheet（美術統一收尾）　　狀態：done（Claude 2026-07-05，全部接入，美術 100% 統一）

最早期的三種敵人還在用第一代主圖集，風格與新美術不一致。請各生一張 12×4 綠幕(#00ff00) sheet（規格同 REQ-008：每列12幀、128px、腳底錨點一致，row0 idle / row1 action / row2 hit / row3 death）：

- **ghoul 魑魅**（小型怨靈雜兵、暗青色調、藍綠眼）→ `survivor/assets/incoming/ghoul/ghoul-greenscreen.png`；action=撲咬
- **mage 咒師**（遠程施法怨靈、粉紫色調、持咒印）→ `survivor/assets/incoming/mage/mage-greenscreen.png`；action=施法
- **brute 巨怪**（大型重擊怪、橙褐色調、體型寬）→ `survivor/assets/incoming/brute/brute-greenscreen.png`；action=重擊蓄力

這是美術統一的最後一批。我會用 `normalize-char-sheet.mjs` 接入。

## REQ-010 — 投射物特效圖組（火球/魔彈）　　狀態：done（Claude 2026-07-06，彗星飛行+命中爆裂已接入）

目前敵人/Boss 的投射物是單張火球圖＋程式殘影，玩家反映「圖片飛過去、不順暢、太陽春」。需要**專門為投射物設計的 flicker 圖組**。

**⚠️ 最重要的規格（之前所有動畫頻閃的教訓）**：
- 每一幀的「核心」（球體本體）必須**位置、大小、形狀完全一致**——**只有外圍的火舌/能量絲允許變化**。這樣輪播才是自然的火焰閃爍，不會頻閃。
- 幀與幀之間差異要小（外圍 20% 以內變化），像真實火焰的連續閃動。

**要兩組**，各一張 6×1 橫條（6 幀、每幀 256px 方格、綠幕 #00ff00、**中心錨點**）：

1. **fireball 烈焰彈**（爆靈/一般敵彈）→ `survivor/assets/incoming/proj_fire/proj_fire-greenscreen.png`
   - 水滴形火球、**尖端朝右（+X 方向）**（程式會照飛行方向旋轉）
   - 橙黃核心＋外圍火舌閃爍
2. **darkbolt 暗月魔彈**（Boss/咒師）→ `survivor/assets/incoming/proj_dark/proj_dark-greenscreen.png`
   - 紫粉色能量彈、彗星形尖端朝右、核心白熱＋紫色能量絲閃爍

在 `FROM_CODEX.md` 登記 ready。我會接上：核心穩定輪播＋程式光暈＋火星粒子＋殘影，達到市售遊戲的投射物質感。

## REQ-011 — 主角攻擊姿勢包（連擊系統用）　　狀態：ready

主角攻擊現在是「每擊輪換姿勢」的連擊系統（目前 4 姿勢，取自舊圖）。要一包全新的獨立攻擊姿勢——注意：不是逐幀動畫，是 8 個各自定格的完整姿勢。

規格（教訓：GPT 生不出連貫逐幀，但很會生獨立姿勢）：
- 一張 8x1 橫條、每格 128px、綠幕 #00ff00、腳底錨點一致（腳踩同一條線）
- 同一隻主角（厭世黑貓符師、同比例同鏡頭），8 個明顯不同的攻擊姿勢：
  1.右揮杖斬 2.左揮杖斬 3.前刺 4.過頂下劈 5.單手前推放符 6.雙手聚氣 7.迴旋掃擊 8.拋擲符卡
- 每格是乾淨定格（少量符卡點綴可以，大特效不要——特效由程式疊）
- 放到 survivor/assets/incoming/hero_attacks/hero_attacks-greenscreen.png

我會接成 8 段連擊輪換。在 FROM_CODEX.md 登記 ready。

## 待補（之後回合）

- soul（魂火掉落物）與 talisman 彈道用現行圖即可，不需重生。
